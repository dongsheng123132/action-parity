...
1. **An interface is a projection, not the application.**

   GUI, CLI, TUI, MCP, API, and automation should reveal the same underlying capabilities.

2. **An action should be implemented once.**

   A feature duplicated across interfaces will drift. A shared Action Core gives every interface one source of truth.
...
5. **Parity must be measurable.**

   Claims such as "AI-ready" are not enough. Applications should publish manifests, mappings, exceptions, tests, and reproducible reports.
