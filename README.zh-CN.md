...
| 等级 | 名称 | 要求 |
|---|---|---|
| AP-1 | Core | 动作可发现、有 Schema、可无界面执行，**且指明无头执行的证据**。 |
| AP-2 | Parity | 每条必需绑定都存在且指向可重跑测试，**且每个动作都能被应用之外的进程调用**。 |
| AP-3 | Agent | 具备结构化结果、权限、确认、审计、进度、取消等 Agent 安全能力。 |
| AP-4 | Verified | 发布可验证报告，证明核心、绑定、无障碍和真实 GUI 流程均通过测试。 |

级别逐级包含，不能只购买徽章而绕过测试。报告必须同时给出**声明覆盖率**和**证据覆盖率**两个数字，并把「自己声称的目标」和「实际达成的等级」分开列——只是把清单填满，绝不能读起来像及格。

一个应用如果唯一的非可视入口是它自己的 webview 命令桥（`#[tauri::command]`、`ipcRenderer` 之类），那它拥有的是一套私有调用约定，不是机器入口。[`examples/gui-only`](examples/gui-only/action-parity.json) 就留作这个回归用例：它在 0.2.0 下能拿到 AP-2，现在报 `Achieved: none`。
