...
第三个试点进入正在使用的 U-King 0.9.67，现已统一六个低风险动作：
电脑体检、工具清单、驱动状态、脱敏供应商目录、只读清理扫描和内存体检报告
预览。release 模式的正式 exe 已验证可被 AI 子进程捕获纯 JSON；非法字段在
0 毫秒被拒绝。供应商完整记录仍然留在私有边界内，机器接口只返回
`has_saved_key`，不返回 API Key；体检报告也不再显示 Key 前缀。
